...
bool rcMarkLowAreas(rcContext* ctx, unsigned int height, rcAreaModification areaMod, rcCompactHeightfield& chf)
{
	const int w = chf.width;
	const int h = chf.height;
				 ...
				 rcReal* outVerts, const int maxOutVerts)
{
	...
	for (int i = 0; i < nverts; i++)
	{
		...
		if (bevel && cross < 0.0f)
		{
			if (n+2 > maxOutVerts) //@HG - fix overconservative bounds check
				return 0;
		...
		{
			if (n+1 > maxOutVerts) //@HG - fix overconservative bounds check
				return 0;
...
void rcReplaceBoxArea(rcContext* ctx, const rcReal* bmin, const rcReal* bmax,
	rcAreaModification areaMod, unsigned char filterAreaId,
	rcCompactHeightfield& chf)
{
	rcAssert(ctx);
...
void rcReplaceConvexPolyArea(rcContext* ctx, const rcReal* verts, const int nverts,
	const rcReal hmin, const rcReal hmax, rcAreaModification areaMod, unsigned char filterAreaId,
	rcCompactHeightfield& chf)
{
	rcAssert(ctx);
...
void rcReplaceCylinderArea(rcContext* ctx, const rcReal* pos,
	const rcReal r, const rcReal h, rcAreaModification areaMod, unsigned char filterAreaId,
	rcCompactHeightfield& chf)
{
	rcAssert(ctx);

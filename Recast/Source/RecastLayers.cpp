...
{
	// [break one directional connections, contour tracing gets stuck in infinite loop]
	...
	unsigned short* srcReg, rcLayerRegionMonotone*& regs, int& nregs)
{
	...
	const int MaxSweeps = w * chf.maxCellSpanCount;
		...
		for (int x = borderSize; x < w - borderSize; ++x)
		{
			...
			for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i)
			{
				...
				if (sid == 0xffff)
				{
					...
					{
						ctx->log(RC_LOG_ERROR, "CollectLayerRegionsMonotone: Layer split is too complex, skipping tile! x:%d y:%d spansTotal:%d spansCurrent:%d", x, y, chf.spanCount, c.count);
						return false;
					}
				}
...
// @HG BEGIN: special handling of "no regions"
	if (nregs == 0)
	{
		regs = 0;
									  ...
									  unsigned short* srcReg, rcLayerRegionMonotone*& regs, int& nregs)
{
	...
	rcScopedDelete<rcLayerSweepSpan> sweeps(chunkSize * chf.maxCellSpanCount);
	if (!sweeps)
	{
			...
			for (int y = chunky; y < maxy; ++y)
			{
				...
				for (int x = chunkx; x < maxx; ++x)
				{
					...
					for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
					{
						...
						{
							sid = sweepId++;
							// multiple spans per single X row may result in more sweeps than originally allocated
	...
	rcHeightfieldLayerSet& lset)
{
	...
	rcScopedDelete<unsigned short> spanBuf4(chf.spanCount*4);
	if (!spanBuf4)
	{

...
void duDebugDrawRawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
	...
	for (int i = 0; i < cset.nconts; ++i)
	{
		...
		for (int j = 0; j < c.nrverts; ++j)
		{
            ...
            if ( v[ 3 ] & RC_CONTOUR_FLAG_ADJ_DISJOINT_VERTEX )
            {
                colv = duRGBA( 0, 255, 0, a );
            }
			else if (v[3] & RC_CONTOUR_FLAG_BORDER_VERTEX)
			{
				colv = duRGBA(255,255,255,a);
			}
            else if ( v[ 3 ] & RC_CONTOUR_FLAG_AREA_BORDER )
            {
...
void duDebugDrawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
	...
	for (int i = 0; i < cset.nconts; ++i)
	{
		...
		for (int j = 0, k = c.nverts-1; j < c.nverts; k=j++)
		{
			...
			unsigned int col = (va[3] & RC_CONTOUR_FLAG_AREA_BORDER) ? bcolor : color; 
			duReal fx,fy,fz;
			fx = orig[0] + va[0]*cs;
			fy = orig[1] + (va[1]+1)*ch;
			fz = orig[2] + va[2]*cs;
